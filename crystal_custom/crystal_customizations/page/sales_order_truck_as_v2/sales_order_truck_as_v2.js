frappe.pages['sales-order-truck-as-v2'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Truck Assignment (New)',
		single_column: true,
	});
...
this.margin_panel.find('.btn-close-margin-panel, .nexus-margin-backdrop').on('click', () => {
	...
	add_new_truck() {
		frappe.prompt([
			...
			if (!plate) return;
			this._upsert_truck(plate, vals.driver_name, vals.capacity_kg, false);
		}, __('Add Truck'), __('Add'));
	}

	// Every truck creation/edit path routes through this single call so a truck
	// is always written to the Crystal Truck doctype the moment it's introduced —
	// never held only in this page's in-memory array.
	_upsert_truck(truck_number, driver_name, vehicle_type, force_restart) {
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.upsert_truck',
			args: {
				truck_number, driver_name: driver_name || '', vehicle_type: vehicle_type || null,
				trip_id: this._gen_trip_id(), force_restart: force_restart ? 1 : 0,
			},
			callback: (r) => {
				const res = r.message || {};
				if (res.status === 'needs_confirm') {
					frappe.confirm(
						__('Truck {0} has {1} active order(s) from the current trip. Archive that trip and start a new one?', [truck_number, res.open_order_count]),
						() => this._upsert_truck(truck_number, driver_name, vehicle_type, true)
					);
					return;
				}
				const label = { created: 'Truck {0} added', reactivated: 'Truck {0} reactivated',
					restarted: 'Trip archived — {0} ready for a new load', updated: 'Truck {0} updated' }[res.status] || 'Truck {0} saved';
				frappe.show_alert({ message: __(label, [truck_number]), indicator: 'green' });
				this.load_trucks(() => this.render_view());
			},
		});
	}
    ...
    frappe.call({
        method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.get_fleet_settings',
        callback: (r) => {
...
close_margin_panel() {
	...
	// Trucks are a real doctype (Crystal Truck) now — this just reads the
	// persisted list, no client-side reconstruction/guessing needed.
	load_trucks(done) {
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.get_trucks',
			callback: (r) => {
				this.available_trucks = r.message || [];
				frappe.call({
					method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.get_truck_history',
					callback: (hr) => { this.closed_trucks = hr.message || []; (done || (() => this.load_data()))(); },
					error:    () => { (done || (() => this.load_data()))(); },
				});
			},
			error: () => { (done || (() => this.load_data()))(); },
		});
	...
	// Any truck number seen on an order but missing from the Crystal Truck
	// master (e.g. legacy data from before this doctype existed) gets a
	// persisted record backfilled here, instead of a memory-only stand-in.
	_sync_missing_trucks(truck_numbers, done) {
		const known = new Set(this.available_trucks.map(t => t.truck_number));
		const missing = [...new Set(truck_numbers.filter(Boolean))].filter(t => !known.has(t));
		if (!missing.length) { done(); return; }
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.sync_missing_trucks',
			args: { truck_numbers_json: JSON.stringify(missing) },
			callback: () => this.load_trucks(done),
			error:    () => done(),
		});
	}
	...
	load_data() {
		...
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.get_truck_assignment_orders',
			args: {
				from_date:           from || null,
				to_date:             to   || null,
			...
			callback: (r) => {
				...
				// Orphaned (dispatched-away) and dismantled orders still carry their old
				// truck number. Clear it so they appear in the awaiting list instead of
				// creating a ghost truck panel.
				merged.forEach(o => { if (o.orphaned_from_truck || o.dismantled_from_truck) o.custom_truck_number = ''; });

				this.orders = merged;

				const referenced_trucks = this.orders.map(o => o.custom_truck_number).filter(Boolean);
				this._sync_missing_trucks(referenced_trucks, () => this._load_order_locations());
			},
			error: () => { this.render_view(); },
		});
	}

	_load_order_locations() {
	...
	_filter_table_search(list) {
		if (!this.table_search_term) return list;
		const q = this.table_search_term.toLowerCase();
		return list.filter(o =>
			(o.name                    || '').toLowerCase().includes(q) ||
			(o.customer_name           || '').toLowerCase().includes(q) ||
			(o.customer                || '').toLowerCase().includes(q) ||
			(o.sales_persons           || '').toLowerCase().includes(q) ||
			(o.custom_delivery_region  || '').toLowerCase().includes(q) ||
			(o.dismantled_from_truck   || '').toLowerCase().includes(q) ||
			(o.orphaned_from_truck     || '').toLowerCase().includes(q)
		);
	}

	render_view() {
		const orders     = this.get_filtered_orders();
		const unassigned_all = orders.filter(o => !o.custom_truck_number);
		const unassigned = this._filter_table_search(unassigned_all);
		...
		<div class="ta-kpi-row">
			...
			${this._kpi('Unassigned',     unassigned_all.length,        unassigned_all.length > 0 ? '#ef4444' : '#10b981')}
		...
		<div class="ta-section">
			<div class="ta-section-header">
				...
				<span class="ta-count-badge">${unassigned.length}${this.table_search_term ? ` of ${unassigned_all.length}` : ''}</span>
			</div>
			${this._render_table_search(unassigned_all.length)}
	...
	_render_table_search(total_count) {
		return `
		<div class="ta-table-search-wrap">
			<i class="fa fa-search ta-table-search-icon"></i>
			<input type="text" class="ta-table-search-input" placeholder="Search this table — order, customer, region, truck…"
			       value="${frappe.utils.escape_html(this.table_search_term)}">
			${this.table_search_term ? `<button class="ta-table-search-clear" title="Clear search">&times;</button>` : ''}
		</div>`;
	}

	_render_awaiting(all_orders) {
		if (!all_orders.length) {
			return `<div class="ta-empty">${this.table_search_term ? 'No orders in this table match your search.' : 'All orders have been assigned to trucks.'}</div>`;
		}
		...
		sorted_keys.forEach(key => {
			...
			// Sort: dismantled (just yanked off a truck) → overdue → orphan (needs
			// reassign) → submitted-needs-truck → normal. Dismantled orders rank
			// highest so they surface at the top of the table, not buried.
			const _today = frappe.datetime.get_today();
			grp.sort((a, b) => {
				const _dismantled = o => !!(o.dismantled_from_truck);
				...
				const rank = o => _dismantled(o) ? 0 : _overdue(o) ? 1 : _orphan(o) ? 2 : _submitted(o) ? 3 : 4;
			...
			grp.forEach(o => {
				...
				const is_dismantled = !!(o.dismantled_from_truck); // truck was removed before dispatch
				const today         = frappe.datetime.get_today();
				const is_overdue    = o.delivery_date && o.delivery_date < today
					&& !parseFloat(o.per_delivered || 0)
					&& !parseFloat(o.per_billed    || 0);
				html += `
				<tr class="ta-row${checked ? ' ta-row-selected' : ''}${is_pf ? ' ta-row-pf' : ''}${is_dismantled ? ' ta-row-dismantled' : ''}" data-order="${o.name}">
					<td class="ta-td-chk">
					...
					<td>
						...
						${is_dismantled ? `<div class="ta-dismantled-badge" title="This truck was removed before dispatch — the order was returned here.">&#9888; Dismantled from ${frappe.utils.escape_html(o.dismantled_from_truck)}</div>` : ''}
						...
						${is_submitted && o.delivery_date && !is_overdue && !is_orphan && !is_dismantled ? `<div style="font-size:10px;color:#7c3aed;margin-top:1px;">Due: ${frappe.datetime.str_to_user(o.delivery_date)}</div>` : ''}
					</td>
		...
		if (sel_t) {
			...
			<div class="ta-truck-bulk-bar">
				...
				<div style="display:flex;gap:6px;">
					...
					<button class="btn btn-sm btn-danger  ta-bulk-remove-trucks">Dismantle Selected</button>
				...
				<div class="ta-card-header">
					...
					<div class="ta-truck-btns-container">
						...
						<div class="ta-truck-actions-icons">
							...
							<button class="btn btn-xs btn-danger  btn-delete-truck"  data-truck="${truck.truck_number}" title="${is_empty ? 'Dismantle truck' : 'Dismantle truck — orders return to Awaiting Assignment, flagged'}"><i class="fa fa-trash"></i></button>
						</div>
					</div>
				</div>
	...
	_attach_events() {
		...
		this.container.find('.ta-select-all-chk').on('change', function () {
			...
			const unassigned = self._filter_table_search(self.get_filtered_orders().filter(o => !o.custom_truck_number));
			unassigned.forEach(o => checked ? self.selected_orders.add(o.name) : self.selected_orders.delete(o.name));
			self.render_view();
		});

		// ── Table-scoped search (Awaiting Assignment) ─────────────────────────────
		this.container.find('.ta-table-search-input').off('input').on('input', function () {
			const caret = this.selectionStart;
			self.table_search_term = $(this).val();
			self.render_view();
			requestAnimationFrame(() => {
				const $inp = self.container.find('.ta-table-search-input');
				if ($inp.length) {
					$inp[0].focus();
					try { $inp[0].setSelectionRange(caret, caret); } catch (e) {}
				}
			});
		});
		this.container.find('.ta-table-search-clear').off('click').on('click', function () {
			self.table_search_term = '';
			self.render_view();
		});
		...
		// Truck assignment via text input (on change / blur). Any brand-new plate
		// typed here is persisted as a (bare) Crystal Truck record immediately —
		// details can be filled in later via Edit Truck — rather than only living
		// in this page's in-memory array until some other action happens to save it.
		this.container.find('.ta-truck-input').off('change').on('change', function () {
			const order_name   = $(this).data('order');
			const truck_number = $(this).val().trim().toUpperCase();
			if (truck_number && !self.available_trucks.find(t => t.truck_number === truck_number)) {
				frappe.call({
					method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.ensure_truck_active',
					args: { truck_number },
					callback: () => self.load_trucks(() => self._set_truck(order_name, truck_number)),
				});
			} else {
		...
		this.container.find('.ta-bulk-remove-trucks').off('click').on('click', () => {
			...
			const msg = total_orders
				? __('Dismantle {0} truck(s)? {1} order(s) will return to Awaiting Assignment, flagged so you can see where they came from.', [trucks.length, total_orders])
				: __('Dismantle {0} empty truck(s)?', [trucks.length]);
			frappe.confirm(msg, () => {
				self.selected_trucks.clear();
				self._dismantle_trucks(trucks, 0);
			});
		});
		...
		this.container.find('.ta-pf-btn').off('click').on('click', function () {
			...
			frappe.call({
				method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.set_pre_fulfillment',
	...
	_bulk_assign(truck_number) {
		if (!this.available_trucks.find(t => t.truck_number === truck_number)) {
			frappe.call({
				method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.ensure_truck_active',
				args: { truck_number },
				callback: () => this.load_trucks(() => this._bulk_assign_continue(truck_number)),
			});
			return;
		}
		this._bulk_assign_continue(truck_number);
	}

	_bulk_assign_continue(truck_number) {
		const unassigned = this.get_filtered_orders().filter(o => !o.custom_truck_number);
	...
	_set_truck(order_name, truck_number) {
		...
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.set_truck_number',
	...
	edit_truck_details(truck_number) {
		...
		frappe.prompt([
			...
			frappe.call({
				method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.update_truck',
				args: {
					old_truck_number: truck_number,
					truck_number:     (vals.truck_number || '').trim().toUpperCase(),
					driver_name:      vals.driver_name || '',
					vehicle_type:     vals.capacity_kg || null,
				},
				callback: () => {
					frappe.show_alert({ message: __('Truck updated'), indicator: 'green' });
					this.load_trucks(() => this.load_data());
				},
			});
	...
	// "Dismantle" a truck: the truck is kept as a persisted Crystal Truck record
	// (status=Dismantled) rather than deleted, and any orders still on it are
	// flagged custom_truck_dismantled=1 so they return to Awaiting Assignment
	// visibly tagged instead of silently losing all trace of where they came from.
	delete_truck(truck_number) {
		const truck_orders = this.orders.filter(o => o.custom_truck_number === truck_number);
		const msg = truck_orders.length
			? __('Dismantle truck {0}? {1} order(s) will return to Awaiting Assignment, flagged so you can see where they came from.', [truck_number, truck_orders.length])
			: __('Dismantle truck {0}?', [truck_number]);
		frappe.confirm(msg, () => this._dismantle_trucks([truck_number], 0));
	}

	_dismantle_trucks(truck_numbers, idx) {
		if (idx >= truck_numbers.length) {
			frappe.show_alert({ message: __('Truck(s) dismantled'), indicator: 'green' });
			this.load_trucks(() => this.load_data());
			return;
		}
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.dismantle_truck',
			args: { truck_number: truck_numbers[idx] },
			callback: () => this._dismantle_trucks(truck_numbers, idx + 1),
			error:    () => this._dismantle_trucks(truck_numbers, idx + 1),
		});
	}
	...
	_close_truck_batch(truck_number, truck_orders) {
		...
		const finish = () => {
			...
			// Point the truck's Crystal Truck record at its fresh trip/plan for the next load
			frappe.call({
				method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.set_truck_trip',
				args: { truck_number, trip_id: new_trip_id, plan_name: new_plan_name },
			});

			frappe.show_alert({ message: __('Trip dispatched for {0} — ready for next load', [truck_number]), indicator: 'green' });

			// Refetch both the truck list and trip history — load_data() alone
			// never touches either, so without this a freshly dispatched trip
			// stays invisible in the Trip History tab until a full page reload.
			this.load_trucks(() => this.load_data());
		};

		// 1. Batch-set custom_truck_closed=1 on all Sales Orders
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.close_truck_orders',
		...
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.dispatch_and_restart_truck',
			args: {
				...
				frappe.call({
					...
					callback: (r) => {
						...
						const finish = () => {
							...
							frappe.call({
								method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.save_closed_trucks',
								args: { closed_trucks_json: JSON.stringify(this.closed_trucks) },
							});

							// Make sure the truck exists and is Active — it may since have
							// been dismantled, or (for very old data) never been tracked.
							frappe.call({
								method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.ensure_truck_active',
								args: { truck_number: ct.truck_number, driver_name: ct.driver_name || '', vehicle_type: ct.capacity_kg || null },
								callback: () => {
									frappe.show_alert({
						...
						const next = () => {
							...
							frappe.call({
								method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.set_truck_closed',
...
// ── Spatial / Routing Logic (Leaflet + Nexus Backend) ─────────────────────
	...
	_execute_route_mapping(orders, vehicle_type, container_id, truck_number = null) {
		frappe.call({
			method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.get_company_coordinates',
			callback: (r) => {
				...
				frappe.call({
					method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.calculate_route_proxy',
					...
					callback: (route_r) => {
						...
						if (truck_number) {
							...
							frappe.call({
								method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.reorder_truck_plan_orders',
						...
						if (vehicle_type || truck_number) {
							frappe.call({
								method: 'crystal_custom.crystal_customizations.page.sales_order_truck_as_v2.sales_order_truck_as_v2.get_vehicle_routing_economics',
	...
	_styles() {
		...
		/* Table-scoped search (Awaiting Assignment) */
		.ta-table-search-wrap {
			position: relative;
			margin-bottom: 14px;
			max-width: 420px;
		}
		.ta-table-search-icon {
			position: absolute;
			left: 12px;
			top: 50%;
		...
		.ta-table-search-clear:hover { color: #475569; }

		/* Awaiting table */
		.ta-table-wrap { overflow-x: auto; max-height: 65vh; overflow-y: auto; border-radius: 6px; }
		.ta-table thead th {
			background: #1e293b;
			color: #fff;
			font-size: 11px;
		...
		.ta-row-dismantled td { background: #fef2f2 !important; }
		.ta-row-dismantled:hover td { background: #fee2e2 !important; }
		.ta-dismantled-badge {
